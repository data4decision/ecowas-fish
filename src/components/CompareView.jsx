...
const CompareView = () => {
  ...
  const regionalAvg = () => {
    const region = { country: "🌍 Regional Avg" };
    selectedIndicators.forEach(key => {
      const total = filteredData.reduce((sum, row) => sum + (row[key] || 0), 0);
      region[key] = +(total / filteredData.length).toFixed(2);
    });
    return region;
  };

  const chartData = [...filteredData, regionalAvg()];
  ...
  const handlePNG = async () => {
    ...
    canvas.toBlob(blob => saveAs(blob, `Compare_${selectedYear}.png`));
  };
  ...
  return (
    <div className="p-6 bg-white rounded shadow">
      <h2 className="text-xl font-bold text-[#0b0b5c] mb-4">Comparative Analytics View</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
          ...
          <select
            ...
            onChange={(e) => handleSelectChange(e, setSelectedIndicators)}
          >
            {indicators.map(i => (
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => handleSelectChange(e, setSelectedCountries)}
          >
            {countries.map(c => (
      ...
      <div className="flex justify-end gap-4 mb-4">
        <button onClick={handleCSV} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Download CSV</button>
        <button onClick={handlePNG} className="px-4 py-2 bg-purple-500 text-white rounded hover:bg-purple-600">Export PNG</button>
        <button onClick={handlePDF} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">Export PDF</button>
      </div>
